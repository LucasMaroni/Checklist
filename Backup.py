...
if st.session_state.etapa == 1:
    ...
    st.session_state.dados['VISTORIADOR'] = os.getlogin()

    tipo_cavalo = st.radio("CAVALO", ["TOCO 4X2", "TRUCADO 6X2", "TRAÇADO 6X4"])
    tipo_rigido = st.radio("RÍGIDO", ["TOCO 4X2", "TRUCADO 6X2", "TRAÇADO 6X4"])
    ...
    if st.button("Avançar para Etapa 2"):
        if all([
            ...
            st.session_state.dados.update({
                ...
                "CAVALO_TOCO": "X" if tipo_cavalo == "TOCO 4X2" else "",
                "CAVALO_TRUCADO": "X" if tipo_cavalo == "TRUCADO 6X2" else "",
                "CAVALO_TRACADO": "X" if tipo_cavalo == "TRAÇADO 6X4" else "",
                "RIGIDO_TOCO": "X" if tipo_rigido == "TOCO 4X2" else "",
                "RIGIDO_TRUCADO": "X" if tipo_rigido == "TRUCADO 6X2" else "",
                "RIGIDO_TRACADO": "X" if tipo_rigido == "TRAÇADO 6X4" else "",
                "CARRETA_2": "X" if tipo_carreta == "2 EIXOS" else "",
                "CARRETA_3": "X" if tipo_carreta == "3 EIXOS" else "",
            })
...
# Etapa 3
    ...
    if st.button("Finalizar Checklist"):
        ...
        # Substituição em tabelas (caso existam)
        ...
        def adicionar_fotos(pdf_base, imagens, saida):
            ...
            for img in imagens:
                ...
                can.save()
                packet.seek(0)
                merger.append(PdfReader(packet))
            merger.write(saida)
            merger.close()
